...
public class insumosDao {
    ...
    public List<insumos> listar() {
        ...
        try (Connection con = ConDB.conectar()) {
            ...
            while (rs.next()) {
                ...
                // 🔸 Recalcular estado según las reglas nuevas
                i.recalcularEstado();
    ...
    public boolean agregar(insumos i) {
        ...
        String sql = "INSERT INTO insumos (nombre, unidad_medida, stock_min, stock_actual, fecha_vencimiento, estado) VALUES (?, ?, ?, ?, ?, ?)";
        try (Connection con = ConDB.conectar()) {
            ps = con.prepareStatement(sql);
            ...
            ps.setDate(5, i.getFecha_vencimiento() != null ? new java.sql.Date(i.getFecha_vencimiento().getTime()) : null);
    ...
    public boolean actualizar(insumos i) {
        ...
        String sql = "UPDATE insumos SET nombre = ?, unidad_medida = ?, stock_min = ?, stock_actual = ?, fecha_vencimiento = ?, estado = ? WHERE id_ins = ?";
        try (Connection con = ConDB.conectar()) {
            ps = con.prepareStatement(sql);
            ps.setString(1, i.getNombre());
            ps.setString(2, i.getUnidad_medida());
    ...
    try (Connection con = ConDB.conectar();
        ...
        while (rs.next()) {
            ...
            if (stockActual <= 0 || stockActual < stockMin) {
                // Si algún insumo no tiene stock suficiente
                FacesContext.getCurrentInstance().addMessage(null,
